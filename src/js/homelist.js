...
const seriesPopulares = document.querySelector(".serie");
...
const agregarSeriesPopulares = async () => {
  const { results } = await obtenerSeriesPopulares();
  results.forEach(({ poster_path, id }) => {
    const html = `
        <img src="https://image.tmdb.org/t/p/original/${poster_path}" id="${id}"
        alt="">`;
    const divSerie = document.createElement("div");
    divSerie.classList.add("swiper-slide", "swiper-slide-active", "imgEventS");
    divSerie.innerHTML = html;
    seriesPopulares.appendChild(divSerie);
  });
};
...
const eventos = () => {
  ...
  const crearHtmlInfo = (
    ...
    trailer(id)
      .then((data) => {
        boton.href = `https://www.youtube.com/watch?v=${data.results[0].key}`;
      })
      .catch((err) => alert(err));
...
btntps.addEventListener("click", () => {
  ...
  const seriesPopulares = async () => {
    cambioSeccionSearch(hero_container, nav, hero, hero2, list);
    const { results } = await obtenerSP();
    results.forEach(({ poster_path, id }) => {
      const html = `
              <img src="https://image.tmdb.org/t/p/original/${poster_path}" id="${id}"
              alt="">`;
      const divUM = document.createElement("div");
      divUM.classList.add("swiper-slide", "hero2__img", "imgEventS");
      divUM.innerHTML = html;
      hero2.appendChild(divUM);
      console.log(hero2);
    });
